...
public class LightningArtist : MonoBehaviour {
    ...
    public Vector3 getCollision(Vector3 input) {
        RaycastHit hit;
        Ray ray;

        ray = new Ray(input, Camera.main.transform.forward);

        Vector3 p = Vector3.zero;

        if (Physics.Raycast(ray, out hit)) {
            p = applyCameraTransformMatrix(hit.point);
            lastHit = p;
        } else {
            p = new Vector3(input.x, input.y, lastHit.z);
        }

        return Vector3.Lerp(p, Camera.main.transform.position, 0.02f);
    }
